...
export default function DistrictSchoolsPage() {
  ...
  const fetchOverview = async () => {
    try {
      const overviewData = await getDistrictOverview('30d');
  ...
  const fetchSchools = async () => {
    ...
    try {
      ...
      const data = await getSchoolsInDistrict(params);
        ...
        <div>
          ...
          <p className="text-text-secondary">
            {overview?.district?.name
              ? `Schools in ${overview.district.name} district`
      ...
      {overview && (
        <div className="flex flex-wrap gap-2">
          <div className="flex items-center gap-2 px-3 py-1.5 bg-white border border-gray-100 rounded-lg shadow-sm">
            ...
            <span className="text-sm font-bold text-text">{overview.schools.total}</span>
          </div>
          <div className="flex items-center gap-2 px-3 py-1.5 bg-white border border-gray-100 rounded-lg shadow-sm">
            <Building2 className="h-3.5 w-3.5 text-blue-600" />
            <span className="text-sm text-text-secondary">Public:</span>
            <span className="text-sm font-bold text-blue-600">{overview.schools.public}</span>
          </div>
          <div className="flex items-center gap-2 px-3 py-1.5 bg-white border border-gray-100 rounded-lg shadow-sm">
            <Building2 className="h-3.5 w-3.5 text-purple-600" />
            <span className="text-sm text-text-secondary">Private:</span>
            <span className="text-sm font-bold text-purple-600">{overview.schools.private}</span>
          </div>
        </div>
      )}
