...
const navItems = [
  {
    section: 'Overview',
    items: [
      { label: 'Dashboard', href: '/nacca', icon: LayoutDashboard },
    ]
  },
  {
    section: 'Curriculum',
    items: [
      { label: 'All Curricula', href: '/nacca/curricula', icon: BookOpen },
    ]
  },
  {
    section: 'Analytics',
    items: [
      { label: 'Analytics', href: '/nacca/analytics', icon: BarChart3 },
      { label: 'Coverage', href: '/nacca/analytics/coverage', icon: Target },
    ]
  },
  {
    section: 'System',
    items: [
      { label: 'Settings', href: '/nacca/settings', icon: Settings },
    ]
  }
];
...
export default function NaccaLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (!loading) {
      if (!user) {
        router.push('/login');
      } else if (!['curriculum_admin', 'super_admin'].includes(user.role)) {
  ...
  if (!user || !['curriculum_admin', 'super_admin'].includes(user.role)) {
    return null;
  }
      ...
      <header className="fixed top-0 left-0 right-0 z-50 h-16 bg-white border-b border-gray-200 flex items-center px-4">
        <div className="flex items-center justify-between w-full">
          <div className="flex items-center gap-3">
            ...
            <Link href="/nacca" className="flex items-center gap-2">
          ...
          <div className="hidden md:flex items-center flex-1 max-w-md mx-4">
            <div className="relative w-full">
              ...
              <input
                type="text"
                placeholder="Search curricula, subjects..."
            ...
            <div key={section.section} className="mb-4">
              ...
              <div className="space-y-1">
                {section.items.map((item) => {
                  const isActive =
                    (item.href === '/nacca' && pathname === '/nacca') ||
                    (item.href !== '/nacca' && pathname?.startsWith(item.href));
