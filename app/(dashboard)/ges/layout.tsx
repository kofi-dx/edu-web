...
const navItems = [
  {
    section: 'Overview',
    items: [
      { label: 'Dashboard', href: '/ges', icon: LayoutDashboard },
    ]
  },
  {
    section: 'Geography',
    items: [
      { label: 'Regions', href: '/ges/regions', icon: MapPin },
      { label: 'Districts', href: '/ges/districts', icon: Building2 },
    ]
  },
  {
    section: 'Schools',
    items: [
      { label: 'All Schools', href: '/ges/schools', icon: Landmark },
    ]
  },
  {
    section: 'Analytics',
    items: [
      { label: 'Attendance', href: '/ges/analytics/attendance', icon: TrendingUp },
      { label: 'Assessments', href: '/ges/analytics/assessments', icon: ClipboardList },
      { label: 'Performance', href: '/ges/analytics/performance', icon: BarChart3 },
    ]
  },
  {
    section: 'System',
    items: [
      { label: 'Users', href: '/ges/users', icon: Users },
      { label: 'Reports', href: '/ges/reports', icon: FileText },
      { label: 'Settings', href: '/ges/settings', icon: Settings },
    ]
  }
];
...
export default function GESLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (!loading) {
      if (!user) {
        router.push('/login');
      } else if (!['government_admin', 'super_admin'].includes(user.role)) {
  ...
  if (!user || !['government_admin', 'super_admin'].includes(user.role)) {
    return null;
  }
        ...
        <div className="flex items-center justify-between w-full">
          ...
          <div className="hidden md:flex items-center flex-1 max-w-md mx-4">
            <div className="relative w-full">
              ...
              <input
                type="text"
                placeholder="Search schools, regions, districts..."
              ...
              >
                ...
                <div className="hidden sm:block text-left">
                  ...
                  <p className="text-xs text-text-secondary capitalize">
                    {user.role === 'super_admin' ? 'Super Admin' : 'Government Admin'}
                  </p>
                </div>
            ...
            <div key={section.section} className="mb-4">
              ...
              <div className="space-y-1">
                {section.items.map((item) => {
                  const isActive =
                    (item.href === '/ges' && pathname === '/ges') ||
                    (item.href !== '/ges' && pathname?.startsWith(item.href));
