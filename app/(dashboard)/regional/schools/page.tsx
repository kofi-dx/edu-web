...
export default function RegionalSchoolsPage() {
  ...
  const [showFilters, setShowFilters] = useState(initialDistrict !== 'all');
  ...
  useEffect(() => {
    fetchSchools();
  }, [page, typeFilter, districtFilter, searchTerm]);

  useEffect(() => {
    setPage(1);
  }, [searchTerm, typeFilter, districtFilter]);

  const fetchInitialData = async () => {
    try {
      const [overviewData, districtsData] = await Promise.all([
        getRegionalOverview('30d'),
        getDistrictsInRegion('30d')
      ]);
  ...
  const fetchSchools = async () => {
    ...
    try {
      ...
      const data = await getSchoolsInRegion(params);
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <Landmark className="h-6 w-6 text-akoma-green" />
            <h1 className="text-2xl font-bold text-text">Schools</h1>
          </div>
          <p className="text-text-secondary">
            {overview?.region?.name
              ? `Schools in ${overview.region.name} region`
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-4 shadow-sm">
          <div className="flex flex-col md:flex-row gap-3">
            ...
            <select
              ...
              className="px-4 py-2.5 rounded-lg border border-gray-200 focus:ring-2 focus:ring-akoma-green focus:border-transparent outline-none transition-all bg-white text-sm min-w-48"
            >
            ...
            {schools.map((school) => (
              ...
              >
                ...
                <div className="flex items-center gap-2 text-xs text-text-secondary mb-3">
                  ...
                  <span className="truncate">{school.district || 'No district'}</span>
                </div>
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-between bg-white rounded-xl border border-gray-100 px-6 py-4 shadow-sm">
              <p className="text-sm text-text-secondary">
                Page {page} of {totalPages} • Showing {schools.length} of {total.toLocaleString()} schools
              </p>
