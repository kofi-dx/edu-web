...
const navItems = [
  {
    section: 'Overview',
    items: [
      { label: 'Dashboard', href: '/regional', icon: LayoutDashboard },
    ]
  },
  {
    section: 'Management',
    items: [
      { label: 'Districts', href: '/regional/districts', icon: Building2 },
      { label: 'Schools', href: '/regional/schools', icon: Landmark },
    ]
  },
  {
    section: 'Analytics',
    items: [
      { label: 'Attendance', href: '/regional/analytics/attendance', icon: TrendingUp },
      { label: 'Assessments', href: '/regional/analytics/assessments', icon: ClipboardList },
      { label: 'Performance', href: '/regional/analytics', icon: BarChart3 },
    ]
  },
  {
    section: 'System',
    items: [
      { label: 'Users', href: '/regional/users', icon: Users },
      { label: 'Reports', href: '/regional/reports', icon: FileText },
      { label: 'Settings', href: '/regional/settings', icon: Settings },
    ]
  }
];
...
export default function RegionalLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (!loading) {
      if (!user) {
        router.push('/login');
      } else if (!['regional_admin', 'super_admin', 'government_admin'].includes(user.role)) {
  ...
  if (!user || !['regional_admin', 'super_admin', 'government_admin'].includes(user.role)) {
    return null;
  }
      ...
      <header className="fixed top-0 left-0 right-0 z-50 h-16 bg-white border-b border-gray-200 flex items-center px-4">
        <div className="flex items-center justify-between w-full">
          <div className="flex items-center gap-3">
            ...
            <Link href="/regional" className="flex items-center gap-2">
          ...
          <div className="hidden md:flex items-center flex-1 max-w-md mx-4">
            <div className="relative w-full">
              ...
              <input
                type="text"
                placeholder="Search districts, schools..."
              ...
              <div className="space-y-1">
                {section.items.map((item) => {
                  const isActive =
                    ...
                    (item.href !== '/regional' && pathname?.startsWith(item.href));
